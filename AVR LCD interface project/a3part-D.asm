...
start:
	...
	sts CURRENT_CHAR_INDEX, r26 ; initialize CURRENT_CHAR_INDEX to 0
...
display_u:
	...
	ldi r20, 0
	lds r21, CURRENT_CHAR_INDEX ; display TOP_LINE_CONTENT at specified index
	push r20 ;row
	push r21 ;column
...
timer4:
	...
	cpi r19, 'L'
	breq pressed_left ; if left was pressed, branch to pressed_left

	cpi r19, 'R'
	breq pressed_right ; if right was pressed, branch to pressed_right

	; no button was pressed, exit
	rjmp end
...
; runs if left was pressed
pressed_left:
	; check bounds
	lds r22, CURRENT_CHAR_INDEX
	cpi r22, 0
	brlt end ; exit if already at left-most location

	; arrived here, not out of bounds
	dec r22
	sts CURRENT_CHAR_INDEX, r22 ; decrement CURRENT_CHAR_INDEX

	rjmp end

; runs if right was pressed
pressed_right:
	; check bounds
	lds r22, CURRENT_CHAR_INDEX
	cpi r22, 0x10
	brsh end ; exit if already at right-most location

	inc r22
	sts CURRENT_CHAR_INDEX, r22 ; increment CURRENT_CHAR_INDEX
